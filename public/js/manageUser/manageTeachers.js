...
function loadDataTeachers() {
    ...
    $.ajax({
        ...
        url: "http://localhost:8000/api/loadDataTeachers",
        dataType: 'json',
        success: function(customer) {
            ...
            if (JSON.stringify(customer) != 'false') {
                ...
                for (var i = 0; i < customer.length; i++) {
                    ...
                    cell1.innerHTML = '<td >' + customer[i].teacher_ID + '</td>';
                    ...
                    cell8.innerHTML = '<td >' + '<button class="button" style="vertical-align:middle" onclick="selectDataUpdateTeacher()">' + '<span>' + 'Edit' + '</span>' + '</button>' + '</td>';
...
function insertDataTeachers() {
    ...
    if (teacher_ID == "") {
        ...
        $.ajax({
            type: "POST",
            contentType: "application/json",
            url: "http://localhost:8000/api/insertDataTeachers",
            ...
            success: function(customer) {
                ...
                if (JSON.stringify(customer) != 'false') {
                    ...
                    window.location.href = "/main/manageUsers/manageTeacher/manageTeacher.html";
                } else {
...
function initTeacher() {
    ...
    document.getElementById("typeUI").innerHTML = window.localStorage.getItem("type");
    document.getElementById("yearUI").innerHTML = window.localStorage.getItem("year");
    var teacher_ID = location.search.substring(1);
    ...
    $.ajax({
        type: "POST",
        contentType: "application/json",
        url: "http://localhost:8000/api/initTeachers",
        data: JSON.stringify(data),
        dataType: 'json',
        success: function(customer) {
            var result = JSON.stringify(customer);
            ...
            if (JSON.stringify(customer) != 'false') {
                ...
                document.getElementById("teacher_ID").value = customer[0].teacher_ID
...
function selectDataUpdateTeacher() {
    array = [];
    var table = document.getElementById("listTable"),
        rIndex;
    for (var i = 0; i < table.rows.length; i++) {
        table.rows[i].onclick = function() {
            ...
            window.location.href = "/main/manageUsers/manageTeacher/manageTeacherEdit.html?" + array[0];
        }
    }
}
    ...
    for (var i = 0; i < table.rows.length; i++) {
        table.rows[i].onclick = function() {
            ...
            $.ajax({
                type: "POST",
                contentType: "application/json",
                url: "http://localhost:8000/api/deleteToTableTeacher",
                data: JSON.stringify(data),
                dataType: 'json',
                success: function(customer) {
                    var result = JSON.stringify(customer);
...
function updateTeacher() {
    ...
    if (teacher_ID == "") {
        ...
        $.ajax({
            type: "POST",
            contentType: "application/json",
            url: "http://localhost:8000/api/updateTeacher",
            data: JSON.stringify(data),
            dataType: 'json',
            success: function(customer) {
                var result = JSON.stringify(customer);
                console.log(result);
                if (JSON.stringify(customer) != 'false') {
                    alert("updateTeacher Successful!")
                    window.location.href = "/main/manageUsers/manageTeacher/manageTeacher.html";
                } else {
                    alert("updateTeacher Incorrect!");
